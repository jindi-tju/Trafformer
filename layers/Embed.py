...
class TokenEmbedding(nn.Module):
    def __init__(self, c_in, d_model):
        ...
        self.tokenConv = nn.Conv1d(in_channels=c_in, out_channels=d_model, 
                                    kernel_size=3, padding=padding, padding_mode='replicate')  # TODO 这里用循环式的合适吗 circular
...
class TimeFeatureEmbedding(nn.Module):
    def __init__(self, d_model, embed_type='timeF', freq='h'):
        ...
        freq_map = {'h':4, 't':5, 's':6, 'm':1, 'a':1, 'w':2, 'd':3, 'b':3, "10min":4}  # TODO 3 to 2
        d_inp = freq_map[freq]
...
class DataEmbedding(nn.Module):      # embed_type=TimF                          args.num_nodes
    ...
    def forward(self, x, x_mark=None):
        ...
        x = self.value_embedding(x) + self.position_embedding(x)  # + self.wr_node_embedding(x) # [134, 144, 512], [1, 144, 512]
        if x_mark is not None:
            x = x + self.temporal_embedding(x_mark)  # [134, 144, 512]
